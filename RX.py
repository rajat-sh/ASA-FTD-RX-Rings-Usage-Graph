...
def process_blocks_in_file(file_path):
    ...
    try:
	    ...
	    # Iterate in reverse to avoid index shifting issues while removing elements
            if len(rx_elements) > 50:
	    	# Pair percentages with elements, sort by percentage descending.
                pairs = sorted(zip(rx_percentages, rx_elements), key=lambda x: x[0], reverse=True)

    		# Keep only the top 30.
                top_pairs = pairs[:30]

    		# Overwrite the original lists to keep them aligned.
                rx_percentages[:] = [p for p, _ in top_pairs]
                rx_elements[:] = [e for _, e in top_pairs]

            # Create bar graph
            plt.figure(figsize=(10, 6))
            plt.bar(rx_elements, rx_percentages, color='blue')
            plt.xlabel(f'RX_Interface_data_{i}')
